...
for user, send_ids, take_ids, N, M in wishes:
    ...
    active = h.addBinary()
    h.addConstr(sum(out_vars) <= len(out_vars) * active)
    h.addConstr(sum(in_vars) <= len(in_vars) * active)

    # Total outgoing (sent) <= N if combo is active, make sure at least 1 item is sent
    h.addConstr(sum(out_vars) <= N * active)
    h.addConstr(active <= sum(out_vars))

    # Total incoming (received) >= M if combo is active
    h.addConstr(M * active <= sum(in_vars))
